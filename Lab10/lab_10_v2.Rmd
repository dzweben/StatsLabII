...
```{r, eval=F}
LONGDATAFRAME<-pivot_longer(WIDEDATAFRAME,cols=c("VARNAME1","VARNAME2",...,"VARNAME3"),
                            names_to="CONDITIONVARIABLE",
                            values_to="VALUEVARIABLE")
LONGDATAFRAME
```
...
```{r,eval=F}
LONGDATAFRAME$CONDITIONVARIABLE<-factor(LONGDATAFRAME$CONDITIONVARIABLE)
```
...
Syntax if using wide data:

```{r, eval=F}
pastecs::stat.desc(WIDEDATAFRAME,norm=T)
```

Syntax if using long data (it should not  matter if your condition variable is a factor variable or numeric variable:

```{r,eval=F}
tapply(LONGDATAFRAME$VALUESVARIABLE,LONGDATAFRAME$CONDITIONVARIABLE,pastecs::stat.desc,norm=T)
```
...
```{r,eval=F}
MODELOBJECT<-aov_car(VALUEVARIABLE~CONDITIONFACTORVARIABLE+Error(IDVARIABLE/CONDITIONVARIABLE),data=LONGDATAFRAME)
summary(MODELOBJECT)
```
...
```{r,eval=F}
SUBSETTEDDATAFRAME<-subset(WIDEDATAFRAME,select=c(VARNAME1,VARNAME2,...,VARNAMELAST))
```
...
```{r,eval=F}
CMDATAFRAME<-as.data.frame(cm.ci(data.frame=SUBSETTEDDATAFRAME,
                                  conf.level=.95,
                                  difference=FALSE))
```
...
```{r,eval=F}
CMDATAFRAME$CONDITIONFACTORVAR<-rownames(CMDATAFRAME)
```
...
```{r, eval=F}
LONGDATAFRAME<-group_by(LONGDATAFRAME,CONDITIONNUMERICVAR,CONDITIONFACTORVAR)
MEANDATAFRAME<-summarize(LONGDATAFRAME,MEANVARIABLE=mean(VALUESVARIABLE,na.rm=TRUE))
MEANDATAFRAME
```
...
```{r,eval=F}
DATAFRAMEFORGRAFIN<-merge(MEANDATAFRAME,CMDATAFRAME,by="CONDITIONFACTORVAR")
DATAFRAMEFORGRAFIN
```
...
```{r,eval=F}
EMMGRIDOBJECT<-emmeans(MODELOBJECT,~CONDITIONFACTORVARIABLE)
EMMGRIDOBJECT
```
...
```{r, eval=F}
VECTORNAME<-c(-2.5,-1.5,1.5,2.5)
LISTNAME<-list(VECTORNAME)
contrast(EMMGRIDOBJECT,LISTNAME)
```
...
Syntax (to get the t-critical value; note you have to customize the `a-1`, `A-1`, and `n-1` arguments!)

```{r, eval=F}
sqrt((a-1)*qf(1-ALPHA,(a-1),(n-1)))
```
