...
Normality looks acceptable across conditions (Shapiro-Wilk tests are not extreme for most levels), but sphericity is imperfect, so the Greenhouse-Geisser correction should be used in reporting.
...
ggplot(data = plot_df, aes(x = letters_num, y = mean_solved, group = 1)) +
  geom_line() +
...
```{r p15_output, echo=FALSE, fig.cap="Puzzles solved by number of letters"}
...
```{r p15_save_plot, include=FALSE}
if (!dir.exists("lab10_output")) dir.create("lab10_output")
ggsave(filename = file.path("lab10_output", "fig_means.png"), plot = plot_obj, width = 7, height = 4.5, dpi = 200)
```
...
num_df <- a - 1
den_df <- (n - 1) * (a - 1)
# epsilon = adjusted df / original df
